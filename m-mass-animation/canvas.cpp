...
*/
            cr->set_source_rgb((a.x+570)/1140,(a.y+570)/1140,abs(a.x+a.y)/2280);
 //       if ( i == 1 )
      //      cr->arc(a.x + sin(2*M_PI*m_dAnimatorRot)*125, a.y + cos(2*M_PI*m_dAnimatorRot)*125, a.r, 0, 2*M_PI);
            double l=sqrt(pow(a.x,2)+pow(a.y,2));
            auto const x{a.x + sin(2*M_PI*m_dAnimatorRot)*(l*sin(a.x/570*M_PI))};
            auto const y{a.y + cos(2*M_PI*m_dAnimatorRot)*(l*cos(a.y/570*M_PI))};
            cr->arc(x, y, a.r, 0, 2*M_PI);
/*
        else if ( i == 2 )
