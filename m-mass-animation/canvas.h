...
class CCanvas : public Gtk::DrawingArea
    ...
    public:
        CCanvas()
            ...
            m_afConnections[0] = Glib::signal_timeout().connect(m_fSlot[0], 5);
            m_afConnections[1] = Glib::signal_timeout().connect(m_fSlot[1], 5);
            for (double x{-570}; x < 570; x+=40) 
               for (double y{-570}; y < 570; y+=40) 
                    m_vFlecken.emplace_back( SFleck{x,y,20} );
            }
